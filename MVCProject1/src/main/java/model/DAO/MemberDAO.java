...
public class MemberDAO extends DataBaseInfo{
	...
	public void memberUpdate(MemberDTO dto) {
		...
		String sql = " update member "
				+ " set mem_name = ?, mem_phone1 = ?, mem_phone2 = ?,"
	...
	public void memberJoin(MemberDTO dto) {
	      ...
	      String sql = "insert into member ( " + COLUMNS +") "
	            ...
	            + " ? , ? , ? , ? , ?, ?, ?) ";
	      try {
	         pstmt=con.prepareStatement(sql); // sql문 날리기
	         pstmt.setString(1, dto.getMemName());
	         ...
	         int i = pstmt.executeUpdate(); //얘가 있어야  위의 값을 주고 실행이 된다. 
	         System.out.println(i + "개 행이(가) 삽입되었습니다.");
	...
	public void memberPassUpdate(String memId, String newMemPw) {
		con = getConnection();
		String sql = " update member"
				+" set mem_pw = ?"
				+" where mem_id = ?";
